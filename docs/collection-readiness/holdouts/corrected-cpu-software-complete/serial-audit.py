...
import collections, datetime, fcntl, hashlib, json, math, pathlib, subprocess, sys
...
checks = {}; cells = []; sources = {}; validity = collections.Counter(); phases = collections.Counter()
for cell in plan['cells']:
    ...
    for run in group['runs']:
        ...
        # Decode every retained file afresh, including identical-byte repetitions.
        command = [receipt['runtimeVerification']['ffprobePath'], '-v', 'error', '-threads', '1', '-select_streams', 'v:0', '-count_frames', '-show_entries', 'stream=nb_read_frames,width,height,avg_frame_rate,duration', '-of', 'json', str(artifact)]
...
assert len(cells) == 42
summary = {'status': 'COMPLETE_CORRECTED_SOFTWARE_TIMING_AUDIT', 'inspectedAt': datetime.datetime.now(datetime.timezone.utc).isoformat(), 'sourceCommit': plan['sourceCommit'], 'sourceArchiveSha256': plan['sourceArchiveSha256'], 'planHash': claimed, 'physicalSources': 1, 'cellCount': len(cells), 'stableCells': sum(c['stability']['stable'] for c in cells), 'unstableCells': sum(not c['stability']['stable'] for c in cells), 'phaseCounts': dict(phases), 'validityCounts': dict(validity), 'referenceSha256': sources, 'cells': cells, 'limitations': ['Timing and byte/frame coverage only; no authoritative VMAF/XPSNR analysis, ranking, import, or human review performed by this audit.', 'Previous 17 observations from the affected sampler revision remain exploratory and excluded.', 'Hardware extension and canonical calibration matrix are separate, unexecuted phases.']}
