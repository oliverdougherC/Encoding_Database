...
- Source/encoded counts must match the complete clip. Wall time is positive finite floating-point milliseconds up to 24 h; FPS and real-time ratio must agree with canonical count/duration and wall time within 0.2% (0.001 absolute floor). Source cadence must match the exact rational manifest. Old measurements are not rewritten or relabeled.
...
- Analysis leases renew every third of the lease interval. Completion/retry/failure lock the analysis row before evaluating the current claim with `clock_timestamp()`; renewal uses the same database-clock discipline. Shutdown aborts active claim controllers, including future native phases, and an awaited claim cannot launch after stop. Upload claims return a database-owned absolute deadline after lock acquisition, and body processing uses only its remaining duration. Expired poison jobs terminate at the attempt limit. Aggregate failure does not erase successful immutable analysis: `recomputePending` persists and retries separately, including uploaded work interrupted before queue insertion.
...
PostgreSQL 16 was an isolated Docker database `encodingdb_backend`, with all 26 backend migrations applied. The test refuses a differently named backend database. No production database or artifact volume was touched.

The fault suite runs real PostgreSQL contention from two independent Prisma connections, exact fractional-client timer roundtrip (1990.073417 ms), deliberately blocked upload/analysis locks crossing lease expiry, shutdown between native phases and during claim acquisition, 12 identical creates, 25 concurrent admissions at cap two, immutable conflicts, reservation expiry/drain, quota/near-full rollback, globally bounded slots, duplicate active enqueue, explicit stale completion/retry rejection, lease renewal across a slow valid job, poison-worker expiry, transport recovery, durable rebuild retry, and deliberately out-of-order aggregate snapshots. It also appends an older synthetic EXPECTED review and verifies that review time cannot select the old analysis, and that a newer PENDING analysis blocks it.
...
## Review follow-up: UTC, active contexts, installed provenance and reported validity

The final follow-up passes **190 server tests, zero failures, zero skips**. The database lease expression is now explicitly UTC-naive, `(clock_timestamp() AT TIME ZONE 'UTC')`, matching Prisma's `TIMESTAMP WITHOUT TIME ZONE` representation. Queue, artifact and run creation explicitly use the same UTC clock; existing historical rows are not rewritten. The full PostgreSQL fault suite additionally passes in `America/Los_Angeles` and `Asia/Kolkata`, with blocked expiry and timestamp chronology assertions. Reproduce the focused tests with `BACKEND_TEST_DATABASE_URL` pointing to the isolated migrated database and `BACKEND_TEST_TIMEZONE` set to either zone, then run `node --test test/v7-backend-faults.test.js test/v7-worker-provenance.test.js`. Separate TAP receipts retain those results.

Recomputation selects only the validated, explicitly configured PRODUCTION context version/formula/model/hash. No configured active epoch means no historical rewrite. A real database A/B regression adds an analysis, reviews the B analysis, restarts the callback through another Prisma connection and verifies that B rebuilds while A's rows, membership and policy remain unchanged. Exact historical review dependencies can still be withdrawn by the review helper; unrelated historical contexts are preserved.

Authoritative provenance now contains `workerBuildFingerprint`, `workerModuleFingerprint`, the hashed relative compiled-module closure, and `installedTools.ffmpeg` / `installedTools.ffprobe` with resolved executable path, SHA256 and byte size. Hashing streams installed bytes; native execution uses the same resolved executable. Cached identities reject executable replacement during a live process and refresh on restart. Tests change binary contents without changing its reported version, then change a compiled module, and verify that fresh processes produce changed fingerprints grounded in the installed files. No caller-supplied provenance is accepted. The earlier seven-clip receipts are preserved as originally recorded, rather than retroactively assigning them new build identities; metric computation has not changed and that long trial was not rerun for metadata changes.

Measurement disposition also retains the client's three protocol validity reports. Invalid reports produce INVALID runs, which cannot be promoted by an EXPECTED review. Suspect, missing or unknown reports produce SUSPECT runs even when the pixel analysis is COMPLETE. Valid reports preserve the server's metric disposition and are explicitly described as self-reports, not remote attestation. The raw reports and automated quality status/flags remain intact. Synthetic valid test payloads now explicitly include all three valid states; regressions cover suspect/invalid/missing reports and contradictory invalid reasons.
