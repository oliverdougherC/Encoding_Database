...
Certificate v3 hashes each complete row state before aggregating ordered `(run ID, digest)` pairs. SQL `COLLATE C` provides identical publication and verification ordering, including namespaced Unicode IDs. A materialized page certificate prevents PostgreSQL from evaluating the full certificate twice through join-filter pushdown. All original state fields, full-group scope, effective review checks and dependency invalidation remain covered. The restart dispatcher detects old certificate versions for the explicitly validated active context even when dependency rows are clean; it revalidates and rebuilds their exact retained membership. Historical contexts are not rewritten. Existing active-artifact behavior-hash validation remains enforced.
