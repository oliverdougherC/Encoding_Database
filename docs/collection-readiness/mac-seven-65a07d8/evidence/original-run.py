...
output=root/'.build/native-seven-mac'
cli=root/'.build/release-20260914/macos-65a07d8/encodingdb-client-macos'
state=root/'.build/release-20260914/mac-host-state'
cache=root/'.build/native-faults-20260914/épreuve 客户端/clean-cache'
state.mkdir(parents=True,exist_ok=True)
receipt={'sourceCommit':'65a07d8','executableSha256':hashlib.file_digest(cli.open('rb'),'sha256').hexdigest(),'purpose':'Actual packaged seven-clip native acceptance; no publication and not the predeclared calibration matrix','cases':[]}
def save():
...
with (state/'measurement.lock').open('a+b') as lock:
 ...
 for name,codec,settings,seed in [('software','libx264',['--presets','fast','--crf','23'],20260914),('videotoolbox','h264_videotoolbox',['--presets','default','--target-bitrate-kbps','6000'],20260915)]:
