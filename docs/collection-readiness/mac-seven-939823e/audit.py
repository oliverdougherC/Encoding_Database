...
INPUT = ROOT / '.build/native-seven-mac-939823e/épreuve 客户端'
PACKAGE = ROOT / '.build/release-20260919/macos-939823e'
...
for case in receipt['cases']:
    ...
    for path in base.rglob('*'):
        if path.is_file() and (path.suffix == '.json' or path.name == 'client.log'):
            ...
            copy(path, relative)
    ledger = read(base / 'before-upload.json')
    ...
    assert collections.Counter(a['schedule']['phase'] for a in attempts) == {'warmup': 7, 'measured': 14}
    result = {'name': name, 'campaignId': campaign.name, 'exitCode': case['exitCode'],
              ...
              'runtimeFingerprint': embedded['runtimeLockFingerprint'], 'attempts': [], 'groups': []}
    for attempt in attempts:
        ...
        if info['ffmpegSampleCount'] > 0:
            ...
            assert info.get('ffmpegCpuUtilAvg') > 0 and info.get('ffmpegCpuUtilMax') > 0
        else:
        ...
        result['attempts'].append({'executionOrder': schedule['execution_order'], 'clipId': clip, 'phase': schedule['phase'],
            ...
            'backgroundGpuPct': env['background_gpu_pct'] if env else None, 'processCpuAveragePct': info.get('ffmpegCpuUtilAvg'),
    ...
    for group in protocol['recipeResults']:
        ...
        for attempt in measured:
            ...
            assert len(submission['measurementGroup']['countedAttempts']) == 2
        ...
        result['groups'].append({'recipeId': group['recipeId'], 'measuredSeconds': values, 'relativeSpread': spread,
                                 'stable': spread <= 0.03, 'countedAttempts': 2})
