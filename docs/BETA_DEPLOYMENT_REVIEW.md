...
## PR #12 deployment repair

**REVIEWABLE WITH NAMED BLOCKERS: beta integration and checks on the eventual integrated beta SHA remain pending.** The focused deployment repair passed the real isolated regression at implementation SHA `694f494600c9b1d1731493fd0d2ec0aab56b86b3`. [Execution certificate](canonical-suite/evidence/deployment-repair.json) records exact commands, timestamps, environment, all seven image hashes, container identities, negative cases and limitations. [Executed logs](canonical-suite/evidence/deployment-repair-logs.tar.gz) include the successful run, earlier failed diagnostic runs and individual local release-gate logs; the certificate binds the archive size and SHA-256. This evidence is committed for review, not published as a final release.

`deploy.sh` now provisions the acquisition Python dependencies and pinned FFmpeg/ffprobe in an isolated preparation image, reuses the existing frozen-pack archive/clip/notice/identity checks, materializes both resource trees, builds all application images and pulls service images before rollout. Rollout disables further builds and pulls. `--prepare-only` performs the same preparation without starting services. No source selection, suite identity, server source/model check, metric band, default CRF, or PL setting changed.

The regression ran on 2026-09-13 00:07:02–00:09:37 UTC (September 12 locally). A new Git clone had neither canonical directory, no adjacent pack and an empty suite cache. The actual deployment entry point acquired the committed public pack and started the production database/server/frontend/nginx stack using disposable credentials, unique volumes/network and ephemeral loopback ports. Existing Docker image/build layers were reusable locally; no canonical files or suite cache were copied in. The dedicated CI job also starts from a new clone and has no unrelated materialization setup.

- All four services became healthy; migrations, database query, API readiness/evidence/catalog/corpus, homepage and frontend corpus proxy checks passed.
- An independent container using the resulting server image, with networking disabled and no mounts, verified all seven source sizes/hashes and four metadata files against immutable Git objects. The suite fingerprint remains `d40bff563dead0e78003af90b2626003bd80afcc12220ab86bc6d4a4b8c83b6e`.
- Missing pack, unreachable exclusive mirror, and same-size corrupt archive each failed with its expected diagnostic before the rollout marker. Container IDs, start times, image IDs and restart counts stayed unchanged; no service lifecycle events occurred and no unverified canonical directory was installed.
- After isolated teardown, `--prepare-only` restored all fourteen client/server files from the verified cache and created no services. Final cleanup succeeded.
- Four isolation/smoke safety tests passed. Local hygiene, metadata, production-compose, suite-contract/drift, ops-hooks and final-suite gates passed at the same implementation SHA. Refreshed [application/native CI](https://github.com/oliverdougherC/Encoding_Database/actions/runs/34727143625), [full release preflight](https://github.com/oliverdougherC/Encoding_Database/actions/runs/34727143627) and [dependency audits](https://github.com/oliverdougherC/Encoding_Database/actions/runs/34727143639) identify that head. PR #12 records final review-head results and exact CI checkout identities after completion; a run link alone is not a success claim. CI retains regression and individual release-preflight logs for 90 days.

Execution exposed two additional stale production-path checks: nginx's localhost probe selected an unbound address and did not trust isolated TLS, and the homepage smoke required the retired “Encoding Database” name. The corrected loopback probe uses IPv4 HTTPS with the mounted serving certificate as trust input, without disabling certificate verification; external TLS checks remain intact. Homepage checks now require the shipped `EncodingDB` title and `V7 public corpus` heading. The earlier failures at `3749b92` and `822d08e` are retained and excluded from acceptance; superseded CI was cancelled and replaced by the repaired head's workflows.

This regression proves preparation, isolated startup and blank production PL configuration, with test-only contexts disabled. Its empty corpus is not a new packaged-client collection certification. The retained real collection evidence below, including original CRF24 diagnostics and null-PL rows, remains historical evidence with its own exact binary/source identities. No beta/main merge, production deployment, final release or PL activation occurred.
