...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --api-key-header|--per-key-per-minute|--per-key-per-day|--disk-min-free-gb|--disk-path|--admin-token)
      echo "$(yellow "Warning: $1 is deprecated and ignored (feature not implemented in backend).")" >&2
      shift 2
      ;;
