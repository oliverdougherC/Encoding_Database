...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --software-crf) SOFTWARE_CRF="${2:?}"; shift 2 ;;
    *) echo 'Usage: certify-beta-corpus.sh --client PATH [--suite-pack PATH] [--server-url URL] [--frontend-url URL] [--evidence-root PATH] [--software-crf 0..51]' >&2; exit 2 ;;
  esac
done
[[ "$SOFTWARE_CRF" =~ ^[0-9]+$ ]] && (( 10#$SOFTWARE_CRF <= 51 )) || { echo 'Invalid libx264 CRF; expected 0..51' >&2; exit 2; }
...
while IFS= read -r clip; do
  ...
  env -u ENCODINGDB_SUITE_PACK_PATH -u ENCODINGDB_SUITE_PACK_URL \
    ...
    --v7-suite-clip "$clip" --presets fast --crf "$SOFTWARE_CRF" --retries 1 \
...
python3 - "$RUN_DIR" "$ROOT_DIR" "$CLIENT" "$MANIFEST" "$STARTED_AT" "$SOFTWARE_CRF" <<'PY'
import hashlib,json,pathlib,subprocess,sys
out,root,binary,manifest,started,crf=sys.argv[1:]
def identity(p):
...
data['softwareRateControl']={'encoder':'libx264','mode':'CRF','qualityValue':int(crf)}
