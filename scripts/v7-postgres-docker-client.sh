...
exec docker run --rm --label "encodingdb.backup.scope=${V7_BACKUP_SCOPE:-manual}" --cpus "${V7_BACKUP_DOCKER_CPUS:-2}" --memory "${V7_BACKUP_DOCKER_MEMORY:-2g}" --network "$network" --user "$(id -u):$(id -g)" \
