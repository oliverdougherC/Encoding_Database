...
const assets=join(root,'scripts/validation-screen');
const out=resolve(process.env.SCREEN_OUTPUT || join(root,'.build/evidence/holdouts/validation-screen-30s'));
...
const hashes=[];let capturedBytes=0;
try {
 ...
 for(let frame=0;frame<720;frame++){
   ...
   capturedBytes+=(await stat(file)).size;if(capturedBytes>700*1024*1024)throw Error('Capture exceeds owned700MiB PNG budget');
 ...
 if(errors.length)throw Error(errors.join('\n'));
  ...
  if(hashes.length!==720)throw Error('Expected 720 retained frames');
 }
} finally {await browser.close();}
const video=join(out,'reference.mkv');
const encode=['-hide_banner','-y','-threads','2','-filter_threads','2','-framerate','24','-i',join(out,'frames/%04d.png'),'-frames:v','720','-vf','format=gbrp,zscale=primariesin=bt709:transferin=iec61966-2-1:matrixin=gbr:rangein=full:primaries=bt709:transfer=bt709:matrix=bt709:range=limited,format=yuv420p','-threads','2','-c:v','ffv1','-level','3','-g','1','-slicecrc','1','-color_range','tv','-colorspace','bt709','-color_primaries','bt709','-color_trc','bt709','-an','-fs',String(256*1024*1024),video];
...
const decode=['-hide_banner','-v','error','-xerror','-threads','2','-i',video,'-f','null','-'];
...
const probe=JSON.parse(run(ffprobe,['-v','error','-threads','2','-count_frames','-show_streams','-show_format','-of','json',video]));
const s=probe.streams[0];
if(s.nb_read_frames!=='720'||s.width!==1920||s.height!==1080||s.pix_fmt!=='yuv420p'||s.r_frame_rate!=='24/1'||s.color_range!=='tv'||s.color_space!=='bt709'||s.color_transfer!=='bt709'||s.color_primaries!=='bt709'||Number(probe.format.duration)!==30)throw Error('Unexpected output properties');
await writeFile(join(out,'ffprobe.json'),JSON.stringify(probe,null,2)+'\n');
execFileSync(ffmpeg,['-hide_banner','-y','-threads','2','-filter_threads','2','-i',video,'-vf',"select='eq(n,0)+eq(n,168)+eq(n,312)+eq(n,456)+eq(n,576)+eq(n,696)',scale=640:360,tile=3x2",'-frames:v','1',join(out,'contact-sheet.png')],{stdio:['ignore','ignore','pipe']});
const inputFiles=['workload.html','README.md','../canonical-screen/fonts/IBMPlexMono-Regular.ttf','../canonical-screen/fonts/OFL.txt'];
const inputs=await Promise.all(inputFiles.map(async file=>({file,sha256:await sha(join(assets,file))})));
const metadata={schema:'encodingdb-authored-validation-screen-v1',validationOnly:true,sourceGroupId:'encodingdb-authored-atlas',sceneGroupId:'atlas-receipt-reconciliation-30s',createdAt:new Date().toISOString(),source:'Original offline browser application screen; fictional authored dataset',license:'CC0-1.0 authored content; SIL-OFL-1.1 IBM Plex Mono',fontSource:'https://raw.githubusercontent.com/google/fonts/142c8963e7606b510c93a644c82a4c4cdeae6ef9/ofl/ibmplexmono/IBMPlexMono-Regular.ttf',playwright:{version:pwVersion,modulePath,source:'https://github.com/microsoft/playwright/tree/v1.62.1'},browser:{version:browserVersion,executable,sha256:await sha(executable),revision:'1234',source:'https://cdn.playwright.dev/builds/cft/151.0.7922.34/mac-arm64/chrome-headless-shell-mac-arm64.zip',args},platform:{os:platform(),arch:arch(),node:process.version},viewport:{width:1920,height:1080,deviceScaleFactor:1},timeline:{fps:24,frames:720,durationSeconds:30,events:['log-scroll','filter','detail-drawer','documentation','incremental-search','review-summary'],clock:'frame index / 24'},inputs,captureCreatedAt:previous?.captureCreatedAt || previous?.createdAt || new Date().toISOString(),captureScriptSha256:previous?.captureScriptSha256 || await sha(fileURLToPath(import.meta.url)),preparationScriptSha256:await sha(fileURLToPath(import.meta.url)),tools:{ffmpeg:{path:ffmpeg,sha256:await sha(ffmpeg)},ffprobe:{path:ffprobe,sha256:await sha(ffprobe),version:run(ffprobe,['-version']).split('\n')[0]}},ffmpegVersion:run(ffmpeg,['-version']).split('\n')[0],encodeCommand:[ffmpeg,...encode],decodeCommand:[ffmpeg,...decode],validation:{fullDecodeExitCode:0,frameCount:720,visualReview:'pending integration-owner inspection'},output:{file:'reference.mkv',sha256:await sha(video)},frameHashes:hashes};
await writeFile(join(out,'metadata.json'),JSON.stringify(metadata,null,2)+'\n');
console.log(JSON.stringify({output:video,sha256:metadata.output.sha256,browserVersion,frames:720,fullDecode:'passed'}));
