...
const referencePath = resolveValidationPath(pathBindings, 'REFERENCE', receipt.referencePath, source.sourceSha256, registryDocument.registryHash);
if (await hashFile(referencePath) !== source.sourceSha256 || (await stat(referencePath)).size !== source.byteSize) throw new Error('Registered reference bytes differ');
...
try {
  ...
  for (const { record, measurementGroup } of measured) {
    ...
    const artifactPath = resolveValidationPath(pathBindings, 'ENCODED', info.artifactPath, info.artifactSha256, registryDocument.registryHash);
    if (info.encodeTimerBoundary !== 'ffmpeg-process-v1' || await hashFile(artifactPath) !== info.artifactSha256) throw new Error('Measured artifact or corrected timer differs from journal');
    ...
    const hardware = ['h264_nvenc', 'h264_videotoolbox'].includes(info.encoderUsed);
    const expectedMode = hardware ? 'vbr' : 'crf';
    if (requested.mode !== expectedMode || effective.mode !== expectedMode
      || (hardware && (!Number.isInteger(requested.targetBitrateKbps) || requested.targetBitrateKbps <= 0 || requested.targetBitrateKbps !== effective.targetBitrateKbps))) throw new Error('Controlled native rate control changed or is unsupported');
    const recipe = await db.recipe.upsert({ where: { fingerprint: hash(recipeJson) }, create: {
      ...
      requestedRateControlMode: expectedMode.toUpperCase(), effectiveRateControlMode: expectedMode.toUpperCase(), requestedQualityValue: requested.qualityValue, effectiveQualityValue: effective.qualityValue,
      requestedTargetBitrateKbps: requested.targetBitrateKbps, effectiveTargetBitrateKbps: effective.targetBitrateKbps,
    ...
    const run = await db.benchmarkRun.upsert({ where: { id }, create: { id, benchmarkProtocolId: protocol.id, testClipId: clip.id, workloadId: source.workloadId, recipeId: recipe.id, environmentId: environment.id,
      ...
      preRunEnvironmentCheck: { snapshot: record.environmentSnapshot, overallValidity: record.overallValidity, environmentValidity: record.environmentValidity, structuralValidity: record.structuralValidity, measurementGroup }, clientQualityDebug: { validationOnly: true, originalReferencePath: receipt.referencePath, retainedReferencePath: referencePath, retainedArtifactPath: artifactPath, pathBindingsHash: result.pathBindingsHash, rawJournalRecord: record }, status: 'PENDING' }, update: {} });
    ...
    const temporary = `${destination}.${process.pid}.partial`; await copyFile(artifactPath, temporary);
