...
if echo "$services" | grep -qx "frontend"; then
  compose_exec_node frontend 'const http=require("http"); http.get({host:"127.0.0.1",port:3000,path:"/"},(res)=>{ let body=""; res.setEncoding("utf8"); res.on("data",(chunk)=>body+=chunk); res.on("end",()=>{ if(res.statusCode!==200 || !/<title>EncodingDB<\/title>/.test(body) || !/Compare encoding performance/.test(body)) process.exit(1); process.exit(0); }); }).on("error",()=>process.exit(1));' \
