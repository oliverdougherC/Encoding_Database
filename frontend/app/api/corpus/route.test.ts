...
describe("frontend corpus proxy", () => {
  beforeEach(() => {
  ...
  it("fails fast when upstream is missing and mocks are disabled", async () => {
    const response = await GET(new NextRequest("http://localhost:3000/api/corpus"));
  ...
  it("serves mock data only when ENABLE_QUERY_MOCK=1", async () => {
    ...
    const response = await GET(new NextRequest("http://localhost:3000/api/corpus?limit=1"));
  ...
  it("proxies upstream responses and forwards X-Total-Count", async () => {
    ...
    const response = await GET(new NextRequest("http://localhost:3000/api/corpus?limit=1"));
    expect(fetchMock).toHaveBeenCalledWith("http://backend.test/corpus?limit=1", expect.any(Object));
  ...
  it("returns a stable 502 without exposing upstream exception details", async () => {
    ...
    expect(consoleError).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ upstreamPath: "/corpus", error: expect.any(Error) }),
    );
  });
});
