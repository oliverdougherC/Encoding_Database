...
//
// The four verified collection builds are packaged from the accepted client
// tree at a350d45 content and carry project version 1.3.0-rc.1 / client
// protocol 0.3.0. The intended publication tag is `1.3.0-rc.1` (same
// un-prefixed convention as the historical `1.2.0` tag). The assets become
// downloadable only when the operator publishes them as GitHub release
// assets under that tag and verifies the checksums; the deployment then sets
// COLLECTION_DOWNLOAD_BASE on the frontend service (docker-compose.prod.yml
// passes it through, empty by default). While the variable is unset the page
// must NOT advertise the new URLs or even a release-notes link as live - it
// stages the exact file names and verified checksums instead. Historical
// 1.2.0 (client/0.2.0) assets stay reachable but cannot submit to the server
// version shipped with this page (protocol 7.1).
...
// These are release artifacts, distinct from the frozen b3ef24a calibration
// binary (443e293e...).
