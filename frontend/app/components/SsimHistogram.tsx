...
export default function SsimHistogram({ data, bins = 20 }: { data: Benchmark[]; bins?: number }) {
  const t = useChartTheme();

  const values = useMemo(() =>
    data
      .map((d) => (typeof d.ssim === "number" ? Math.max(0, Math.min(1, d.ssim)) : null))
      .filter((v): v is number => v != null),
    [data]);

  const { binData, lo, hi } = useMemo(() => {
    if (values.length === 0) return { binData: [], lo: 0, hi: 1 };
    let lo = values[0], hi = values[0];
    for (const v of values) { if (v < lo) lo = v; if (v > hi) hi = v; }
    lo = Math.max(0, lo - 0.005);
    hi = Math.min(1, hi + 0.005);
  ...
  const option = useMemo(() => ({
    ...
    tooltip: {
      ...
      formatter: (params: { value: [number, number] }[]) => {
        ...
        const step = (hi - lo) / bins;
        return `SSIM ${(mid - step / 2).toFixed(4)}–${(mid + step / 2).toFixed(4)}<br/><b>${count} run${count === 1 ? "" : "s"}</b>`;
      },
    },
    dataZoom: [
    ...
    series: [
      {
        ...
        emphasis: { itemStyle: { color: "#8aabea" } },
      },
    ],
  ...
  return (
    <div className="card" style={{ padding: 12, height: "100%", display: "flex", flexDirection: "column", boxSizing: "border-box" }}>
      <div style={{ fontWeight: 600, marginBottom: 4, flex: "0 0 auto" }}>SSIM Distribution</div>
