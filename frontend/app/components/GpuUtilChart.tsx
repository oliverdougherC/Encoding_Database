...
export default function GpuUtilChart({ data, title = "Average GPU Utilization by Codec" }: { data: Benchmark[]; title?: string }) {
  const t = useChartTheme();

  const bars = useMemo(() => {
    const sums = new Map<string, { total: number; count: number }>();
    for (const row of data) {
      const util = row.gpuUtilAvg;
  ...
  const option = useMemo(() => ({
    ...
    tooltip: {
      ...
      formatter: (params: { name: string; value: number }[]) =>
        `${escapeHtml(params[0].name)}<br/><b>${params[0].value.toFixed(1)}% GPU</b>`,
    },
