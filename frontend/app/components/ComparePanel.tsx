...
type CompareRow = Benchmark & { _plove: number; _relSize: number; _codecLabel: string };
...
const METRICS: Metric[] = [
  ...
  { label: "PLOVE Score", getValue: r => r._plove > 0 ? r._plove.toFixed(2) : "-", getNumeric: r => r._plove > 0 ? r._plove : null, higherIsBetter: true },
];
...
function findBestIndex(rows: CompareRow[], metric: Metric): number | null {
  ...
  for (let i = 0; i < rows.length; i++) {
    const v = metric.getNumeric(rows[i]);
    if (v == null) continue;
  ...
  if (bestIdx !== null) {
    const vals = rows.map(r => metric.getNumeric(r)).filter(v => v != null);
    if (vals.length > 0 && vals.every(v => v === vals[0])) return null;
  }
  return bestIdx;
}
