...
export default function VmafHistogram({ data, bins = 20 }: { data: Benchmark[]; bins?: number }) {
  const t = useChartTheme();

  const values = useMemo(() =>
    data
      .map((d) => (typeof d.vmaf === "number" ? Math.max(0, Math.min(100, d.vmaf)) : null))
      .filter((v): v is number => v != null),
    [data]);

  const { binData, lo, hi } = useMemo(() => {
    if (values.length === 0) return { binData: [], lo: 0, hi: 100 };
    let lo = values[0], hi = values[0];
    for (const v of values) { if (v < lo) lo = v; if (v > hi) hi = v; }
    // Widen slightly so all bars are visible
    lo = Math.max(0, lo - 1);
  ...
  const option = useMemo(() => ({
    ...
    tooltip: {
      ...
      formatter: (params: { value: [number, number] }[]) => {
        ...
        const step = (hi - lo) / bins;
        return `VMAF ${(mid - step / 2).toFixed(1)}–${(mid + step / 2).toFixed(1)}<br/><b>${count} run${count === 1 ? "" : "s"}</b>`;
      },
    },
    dataZoom: [
    ...
    series: [
      {
        ...
        data: binData,
        barWidth: "95%",
        itemStyle: { color: "#9693CC", borderRadius: [3, 3, 0, 0] },
        emphasis: { itemStyle: { color: "#b3b0e0" } },
      },
    ],
  ...
  return (
    <div className="card" style={{ padding: 12, height: "100%", display: "flex", flexDirection: "column", boxSizing: "border-box" }}>
      <div style={{ fontWeight: 600, marginBottom: 4, flex: "0 0 auto" }}>VMAF Distribution</div>
