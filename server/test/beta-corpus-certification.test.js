...
import { validateBetaSnapshot } from '../scripts/verify-beta-corpus.mjs';
function fixture() {
  ...
  return { manifest: { suiteVersion: 'v1', clips }, snapshot: { runs, serverRows: rows, frontendRows: structuredClone(rows), frontendPage: { ok: true }, plConfiguration: { testOnlyEnabled: false, referenceBitrates: '', referenceVersion: '' }, uploadInterruptionEvidence: { injectedFailures: 1, uploadAttempts: [{ injected: true, status: 503, benchmarkRunId: 'real-0-1' }] }, reanalysis: { status: 202, idempotent: true, benchmarkRunId: 'real-0-1', qualityAnalysisId: 'real-0-1-analysis' } } };
}
