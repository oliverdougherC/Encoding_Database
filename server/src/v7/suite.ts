...
const suiteClipSchema = z.object({
  ...
  source: z.object({
    ...
    redistributionApproved: z.boolean().optional(),
    reviewHash: z.string().trim().min(1).optional(),
  }).strict(),
  acquisition: z.object({
    kind: z.string().trim().min(1),
    ffmpegLavfi: z.string().trim().min(1).optional(),
    container: z.string().trim().min(1),
    videoCodec: z.string().trim().min(1),
    deterministicFlags: z.array(z.string().trim().min(1)).optional(),
    packagedRelativePath: z.string().trim().min(1).optional(),
...
const suiteManifestSchema = z.object({
  ...
  redistribution: z.object({
    ...
    redistributionApproved: z.boolean().optional(),
    reviewHash: z.string().trim().min(1).optional(),
    spdxExpression: z.string().trim().min(1).optional(),
  }).strict(),
